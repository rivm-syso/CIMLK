...
for more information.`;
  ...
  var pluralize$2 = pluralizeExports;
  ...
  const name$1a = "core/post-process";
  ...
  const name$19 = "core/pre-process";
  ...
  const name$18 = "core/base-runner";
  ...
  async function runAll(plugs) {
    ...
    run$17(respecConfig);
    run$14(respecConfig);
    performance.mark(`${name$18}-start`);
    ...
    performance.mark(`${name$18}-end`);
    performance.measure(name$18, `${name$18}-start`, `${name$18}-end`);
  }
...
>
  ...
  const name$16 = "core/location-hash";
  ...
  const name$15 = "w3c/group";

  const W3C_GROUPS_API = "https://respec.org/w3c/groups/";
  async function run$_(conf) {
    if (!conf.group) {
      return;
    }
  ...
  async function getGroupDetails(group) {
    ...
    showError(message, name$15, { hint });
  }
  ...
  const name$14 = "core/templates/show-link";
  ...
  const name$13 = "core/templates/show-logo";
  ...
  function showLogo(logo, index) {
    ...
    if (!logo.alt) {
      ...
      showError(msg, name$13, { hint, elements: [a] });
    }
    if (!logo.src) {
  ...
  const name$12 = "core/templates/show-people";
...
>
  ...
  function personValidator(prop) {
    ...
    return function validatePerson(person, index) {
      ...
      if (!person.name) {
        ...
        showError(msg, name$12, { hint: seePersonHint });
        return false;
      }
  ...
  async function run$Z(conf) {
    ...
    if ((conf.isCGBG || !conf.isNoTrack || conf.isTagFinding) && !sotd.id) {
      ...
      showError(msg, name$11, { elements: [sotd], hint });
    }
...
/>`);
  ...
  function run$R(conf) {
    ...
    if (!Number.isInteger(level) || level < 0) {
      ...
      showError(msg, name$V, { title, elements: [h1Elem] });
      return;
    }
  ...
  function findAbstract() {
    ...
    if (abstract) {
      switch (abstract.localName) {
        ...
        default:
          showError("The abstract should be a `<section>` element.", name$U, {
            elements: [abstract],
          });
          return abstract;
      }
    ...
    // using markdown.
    const searchString = l10n$v.abstract.toLowerCase();
  ...
  const name$T = "core/data-transform";
  ...
  function run$N(conf) {
    ...
    if (norms.length) {
      const sec = createReferencesSection(norms, l10n$u.norm_references);
      refSection.appendChild(sec);
    }
    if (informs.length) {
      const sec = createReferencesSection(informs, l10n$u.info_references);
      refSection.appendChild(sec);
    }
  ...
  function warnBadRefs(refs) {
    for (const { ref } of refs) {
      ...
      showError(msg, name$Q, { hint, elements });
    }
  }
  ...
  function computeExport(dfn) {
    switch (true) {
      ...
      case dfn.matches(".export.no-export"): {
        ...
        showError(msg, name$N, { elements: [dfn], hint });
        break;
      }
  ...
  function validateDefinition(text, type, dfn) {
    ...
    if (entry.requiresFor && !dfn.dataset.dfnFor) {
      ...
      showError(msg, name$N, { hint, elements: [dfn] });
    }

    if (entry.validator) {
      entry.validator(text, type, dfn, name$N);
    }
  }
...
`;
  ...
  // @ts-check
  const name$K = "core/issues-notes";
  ...
  const name$J = "core/best-practices";
  ...
  function decorateFigure(figure, caption, i) {
    ...
    caption.prepend(l10n$o.fig, html`<bdi class="figno">${i + 1}</bdi>`, " ");
  }
  ...
  function decorateTable(table, caption, i) {
    ...
    caption.prepend(l10n$n.table, html`<bdi class="tableno">${i + 1}</bdi>`, " ");
  }
   ...
   *
   */
  const name$G = "core/webidl-clipboard";
...
`;
    ...
    switch (elem.localName) {
      ...
      case "dfn": {
        ...
        if ("export" in elem.dataset) {
          ...
          showError(msg, name$E, { hint, elements: [elem] });
  ...
  function mapTitleToDfns() {
    ...
    for (const key of definitionMap.keys()) {
      ...
      if (duplicates.length > 0) {
        showError(l10n$m.duplicateMsg(key), name$D, {
  ...
  function showLinkingError(elems) {
    elems.forEach(elem => {
      ...
      showWarning(msg, name$D, { title, elements: [elem] });
    });
  }
  ...
  function showErrors({ ambiguous, notFound }) {
    ...
    for (const { query, elems } of notFound.values()) {
      ...
      showError(msg, name$C, { title, elements: elems, hint });
    }
...
`;
   ...
   * that is preferred.
   */
  const name$y = "core/webidl-index";
  ...
  function skipFromToC() {
    ...
    for (const section of sections) {
      ...
      if (maxToc < 0 || maxToc > 6 || Number.isNaN(maxToc)) {
        ...
        showError(msg, name$x, { elements: [section] });
        continue;
      }
...
`;
  ...
  function run$m(conf) {
    ...
    if (!conf.testSuiteURI) {
      showError(l10n$c.missing_test_suite_uri, name$n);
      return;
    }
...
`;
  ...
  const name$i = "core/anchor-expander";
  ...
  function run$h() {
    ...
    for (const a of anchors) {
      ...
      if (!matchingElement) {
        ...
        showError(msg, name$i, { title, elements: [a] });
        continue;
      }
...
`;
  ...
  const name$e = "core/web-monetization";
